...
export function GET(request: NextRequest, { params }: { params: { projectid: string } }) {
    return withSession(request, async (err, session) => {
        ...
        let mappedInsights: DashboardCardData[] = insights.map((data: { insight_id: string, user_id: string, project_id: string, dataset_id: string, title: string, kpi_description: string, chart_type: string, start_date: string, end_date: string, y_range_start: number, y_range_end: number, created_at: string, updated_at: string }) => {
            console.log(data)
            return {
                id: data.insight_id,
                key: data.insight_id,
